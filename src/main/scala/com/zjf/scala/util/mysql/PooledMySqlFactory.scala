package com.bj58.xxzl.scala.util.mysql
...
case class MySqlProxy(jdbcUrl: String, jdbcUser: String, jdbcPassword: String, client: Option[Connection] = None) {
  ...
  /**
    ...
    * @return
    */
  def executeUpdateAuto(insertColumnValueMap: Map[String, Any], tableName: String): Int = {
    var executeSql = " insert into " + tableName + " set "
    val valueList = ArrayBuffer[Any]()
    if (!insertColumnValueMap.isEmpty) {
      for ((key, value) <- insertColumnValueMap) {
        executeSql += key + " = ? ,"
        valueList += value
      }
    }
    val finalExecuteSql = executeSql.substring(0, executeSql.length - 1)
    executeUpdate(finalExecuteSql, valueList.toArray)
  }

  /**
    * 批量执行SQL语句,自动拼接sql
    *
    * @param insertColumnValueListMap
    * @param tableName
    * @return
    */
  def executeBatchAuto(insertColumnValueListMap: Array[Map[String, Any]], tableName: String): Array[Int] = {
    var executeSql = " insert into " + tableName + " set "
    val valueList = ArrayBuffer[Array[Any]]()
    var isFirst = true
    if (insertColumnValueListMap != null && !insertColumnValueListMap.isEmpty) {
      for (singleMap <- insertColumnValueListMap) {
        val singleList = ArrayBuffer[Any]()
        for ((key, value) <- singleMap) {
          if (isFirst) {
            executeSql += key + " = ? ,"
          }
          singleList += value
        ...
        isFirst = false
        valueList.append(singleList.toArray)
      }
    }
    val finalExecuteSql = executeSql.substring(0, executeSql.length - 1)
    executeBatch(finalExecuteSql, valueList.toArray)
  }
...
object CreateMySqlPool {
  ...
  def apply(): GenericObjectPool[MySqlProxy] = {
    ...
    if (this.genericObjectPool == null) {
      this.synchronized {
        // 获取MySQL配置参数
        val jdbcUrl = ConfConstants.JDBC_URL
        val jdbcUser = ConfConstants.JDBC_USER
        val jdbcPassword = ConfConstants.JDBC_PASSWORD
        val size = Integer.parseInt(ConfConstants.JDBC_DATASOURCE_SIZE)
